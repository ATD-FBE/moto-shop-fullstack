...
* **Storage:** Multi-provider system (Local FS / S3 Backblaze) с поддержкой транзакционных роллбэков.
* **Real-time:** Server-Sent Events (SSE) для мгновенных уведомлений и обновлений заказов.
* **Payments:** Интеграция ЮKassa (система адаптеров и оркестраторов).
...
### 🔐 Ролевая модель
* **Администратор:** CRUD категорий/товаров, CRUD новостей/акций, управление пользователями (скидки, бан), рассылка SSE-уведомлений, управление заказами (статус, детали, товары, заметка) и финансами (оплаты/возвраты/аннулирование).
...
3. Настройте файл `.env.development` в `server/config/` (порты, JWT, код админа, БД, S3, ЮKassa).
4. Запустите проект: `npm run dev`.


> **Примечание:** В текущей версии не реализована админская статистика продаж и добавление новых товаров в уже подтвержденный заказ.