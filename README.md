...
### 🔐 Ролевая модель(RBAC)
* **Администратор:** Управление профилем, CRUD категорий/товаров, CRUD новостей/акций, управление пользователями (скидки, бан), рассылка SSE-уведомлений, управление заказами (статус, детали, товары, заметка) и финансами (оплаты/возвраты/аннулирование), просмотр логов ошибок (winston).
...
3. Создайте файл `.env.development` в `server/config/` и заполните его своими данными по образцу `.env.example` (порты, JWT, код админа, БД, S3, ЮKassa).
