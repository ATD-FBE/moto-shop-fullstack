...
export const handleNotificationUpdateRequest = async (req, res, next) => {
    ...
    try {
        ...
        if (!dbNotification) {
            return safeSendResponse(req, res, 404, { message: `Уведомление ${notifLbl} не найдено` });
        }
        if (dbNotification.status !== NOTIFICATION_STATUS.DRAFT) {
            return safeSendResponse(req, res, 400, {
                message: `Уведомление ${notifLbl} уже отправлено, редактирование невозможно`
            });
        }
...
export const handleNotificationSendingRequest = async (req, res, next) => {
    ...
    try {
        const { dbNotification, updateResult } = await runInTransaction(async (session) => {
            ...
            return { dbNotification, updateResult };
        });

        // Отправка SSE-сообщения клиентам-получателям
        if (updateResult.modifiedCount > 0) {
            sseNotifications.sendToClients(dbNotification.recipients, { newUnreadNotificationsCount: 1 });
        }
...
export const handleNotificationMarkAsReadRequest = async (req, res, next) => {
    ...
    try {
        const dbNotification = await Notification.findById(notificationId).lean();
        const notifLbl = dbNotification ? `"${dbNotification.subject}"` : `(ID: ${notificationId})`;
...
export const handleNotificationDeleteRequest = async (req, res, next) => {
    ...
    try {
        ...
        if (!dbNotification) {
            return safeSendResponse(req, res, 404, {
                message: `Уведомление ${notifLbl} не найдено`
            });
        }
        if (dbNotification.status !== NOTIFICATION_STATUS.DRAFT) {
            return safeSendResponse(req, res, 400, {
                message: `Уведомление ${notifLbl} уже отправлено, удаление невозможно`
            });
        }

        await dbNotification.deleteOne();
